    ...
    <div className="min-h-screen px-4 py-8 md:px-6 lg:px-8 max-w-[1600px] mx-auto">
      <nav className="flex items-center justify-between mb-6 pb-4 border-b border-[var(--c-border)]">
        ...
        <div className="flex gap-4 text-xs">
          <Link to="/search" className={`hover:text-[var(--c-accent)] transition-colors ${pathname === '/search' ? 'text-[var(--c-accent)]' : 'text-[var(--c-muted)]'}`}>
            Szukaj
          </Link>
          <Link to="/lessons/01" className={`hover:text-[var(--c-accent)] transition-colors ${pathname.startsWith('/lessons') ? 'text-[var(--c-accent)]' : 'text-[var(--c-muted)]'}`}>
            Lekcje
          </Link>
          <Link to="/cheatsheets/permissions" className={`hover:text-[var(--c-accent)] transition-colors ${pathname.startsWith('/cheatsheets') ? 'text-[var(--c-accent)]' : 'text-[var(--c-muted)]'}`}>
            Cheatsheets
          </Link>
        </div>
      </nav>
      ...
      {segments.length > 0 && (
        <div className="flex items-center gap-1.5 text-[11px] text-[var(--c-muted)] mb-5">
          <Link to="/" className="hover:text-[var(--c-accent)] transition-colors">Hub</Link>
          {segments.map((seg, i) => {
            ...
            return (
              <span key={path} className="flex items-center gap-1.5">
                ...
                {isLast ? (
                  ...
                  <Link to={path} className="hover:text-[var(--c-accent)] transition-colors">{name}</Link>
                )}
              </span>
            )
          })}
